...
Download the link:https://people.apache.org/~dain/openejb-temp/examples[ejb-examples.war] , copy it into the Tomcat webapps directory, and visit link:http://localhost:8080/ejb-examples[] .
...
Simply download the link:https://people.apache.org/~dain/openejb-temp/itests[openejb-itests.war and openejb-standalone-client.jar]  and copy it the war into the Tomcat webapps directory.
...
The easiest way to remove the valve is to simply delete the webapps/openejb/META-INF/context.xml file and the webapps/openejb.war file _while Tomcat is stopped_.
